-- Fix Messages System Foreign Keys
-- The original migration referenced auth.users(id) but the app uses public.users(user_id)

-- Step 1: Drop existing tables if they exist (they may have wrong foreign keys)
DROP TABLE IF EXISTS message_attachments CASCADE;
DROP TABLE IF EXISTS message_recipients CASCADE;
DROP TABLE IF EXISTS messages CASCADE;

-- Step 2: Recreate messages table with correct foreign key to public.users
CREATE TABLE IF NOT EXISTS messages (
  message_id UUID DEFAULT gen_random_uuid() PRIMARY KEY,

  -- Thread management (self-referential)
  ...
  -- Sender - now references public.users(user_id)
...
-- Step 3: Recreate message_recipients table with correct foreign key
...
-- Step 4: Recreate message_attachments table
...
CREATE POLICY "Users can view messages they sent"
  ...
  USING (sender_id IN (SELECT user_id FROM users WHERE user_id = sender_id));
...
CREATE POLICY "Users can insert messages"
  ...
  WITH CHECK (sender_id IN (SELECT user_id FROM users));
...
-- Step 8: RLS Policies for message_recipients
...
CREATE POLICY "Senders can insert recipients"
  ...
  WITH CHECK (
    message_id IN (
      SELECT message_id FROM messages WHERE sender_id IN (SELECT user_id FROM users)
    )
  );
...
-- Admin can manage all recipient records
DROP POLICY IF EXISTS "Admin can manage all recipient records" ON message_recipients;
CREATE POLICY "Admin can manage all recipient records"
  ON message_recipients FOR ALL
  USING (
    EXISTS (
      SELECT 1 FROM users WHERE user_type = 'admin'
    )
  );

-- Step 9: RLS Policies for message_attachments
...
CREATE POLICY "Users can view attachments for their messages"
  ...
  USING (
    message_id IN (
      SELECT message_id FROM messages WHERE sender_id IN (SELECT user_id FROM users)
      UNION
      SELECT message_id FROM message_recipients WHERE user_id IN (SELECT user_id FROM users)
    )
  );
