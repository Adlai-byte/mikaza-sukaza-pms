...
import { guestCreditCardSchema, GuestCreditCard, Guest } from "@/lib/schemas";
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
interface GuestCreditCardDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  guest: Guest;
}

export function GuestCreditCardDialog({ open, onOpenChange, guest }: GuestCreditCardDialogProps) {
  const [creditCards, setCreditCards] = useState<GuestCreditCard[]>([]);
  const [loading, setLoading] = useState(false);
  const [isAdding, setIsAdding] = useState(false);
  const [isEditing, setIsEditing] = useState(false);
  const [editingCard, setEditingCard] = useState<GuestCreditCard | null>(null);
  const [viewingCard, setViewingCard] = useState<GuestCreditCard | null>(null);
  const { toast } = useToast();
  const { logActivity } = useActivityLogs();

  const form = useForm({
    resolver: zodResolver(guestCreditCardSchema),
    defaultValues: {
  ...
  const fetchCreditCards = async () => {
    try {
      ...
      const { data, error } = await supabase
        .from('guest_credit_cards')
        .select('*')
        .eq('guest_id', guest.guest_id);

      if (error) throw error;
      setCreditCards((data || []) as GuestCreditCard[]);
    } catch (error) {
  ...
  const handleAddCreditCard = async (data: any) => {
    try {
      ...
      await logActivity(
        'GUEST_CREDIT_CARD_ADDED',
        { guestId: guest.guest_id, cardType: data.card_type, cardNumber: `****${data.card_number.slice(-4)}` },
        guest.guest_id,
        'Admin'
      );
  ...
  const handleEditCreditCard = async (data: any) => {
    try {
      if (!editingCard?.guest_credit_card_id) return;

      const { error } = await supabase
        .from('guest_credit_cards')
        .update(data)
        .eq('guest_credit_card_id', editingCard.guest_credit_card_id);

      if (error) throw error;

      await logActivity(
        'GUEST_CREDIT_CARD_UPDATED',
        { guestId: guest.guest_id, cardType: data.card_type, cardNumber: `****${data.card_number.slice(-4)}` },
        guest.guest_id,
        'Admin'
      );
  ...
  const startEdit = (card: GuestCreditCard) => {
    setEditingCard(card);
    setIsEditing(true);
    form.reset({
  ...
  const handleDeleteCreditCard = async (cardId: string) => {
    try {
      const { error } = await supabase
        .from('guest_credit_cards')
        .delete()
        .eq('guest_credit_card_id', cardId);

      if (error) throw error;

      await logActivity(
        'GUEST_CREDIT_CARD_DELETED',
        { guestId: guest.guest_id, cardId },
        guest.guest_id,
        'Admin'
      );
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>
            Credit Cards - {guest.first_name} {guest.last_name}
          </DialogTitle>
        </DialogHeader>
                ...
                >
                  ...
                  <div className="flex space-x-2">
                    ...
                    <Button
                      variant="destructive"
                      size="sm"
                      onClick={() => card.guest_credit_card_id && handleDeleteCreditCard(card.guest_credit_card_id)}
                      disabled={!card.guest_credit_card_id}
                    >
                      Delete
                    </Button>
                  </div>
