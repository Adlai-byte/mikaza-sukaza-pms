...
test.describe('Employee Documents Module Tests', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto(ROUTES.employeeDocuments);
    await waitForPageLoad(page);
  });

  test('EMP-001: Should load employee documents page', async ({ page }) => {
    await expect(page.locator('text=Employee').first()).toBeVisible({ timeout: 15000 });
  });

  test('EMP-002: Should display stats cards', async ({ page }) => {
